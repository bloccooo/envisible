...
- [ ] **Fix 2 — distinguish "remote unavailable" from "remote empty" and surface it** (high) — `pull()` now returns a `PullOutcome { doc, remote_unreachable }` (`lib/src/vault_repo.rs`), and the TUI shows a `⚠ OFFLINE — showing cached data` indicator in the header + a yellow border whenever the most recent pull couldn't reach remote storage (`cli/src/tui/components/header.rs`, `state.rs::offline`, wired through `ui.rs` on load and `Actions::Sync` on manual sync). `envi exec`/`envi sync`/`envi setup` still don't surface it since they have no persistent UI. Still open: `pull()` silently falls back to `init_vault()` if truly nothing is found anywhere — no dedicated error for that case yet.
