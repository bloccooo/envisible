...
fn generate_and_parse_invite_roundtrip() {
    ...
    let link = generate_invite(&storage, workspace).unwrap();
    let payload = parse_invite(&link).unwrap();
    assert_eq!(payload.workspace.id, "ws-123");
    assert_eq!(payload.workspace.name, "My Workspace");
}
...
fn invite_link_starts_with_prefix() {
    let link = generate_invite(
        &fs_storage(),
        WorkspacePayload { id: "id".to_string(), name: "name".to_string() },
    )
    .unwrap();
